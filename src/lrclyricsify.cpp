...
void get_page(string url, const char *file_name) {
  ...
  curl_easy_setopt(
      easyhandle, CURLOPT_SSLVERSION,
      3); // not sure if this line is needed, and whether it would do any good
  fclose(file);
}
...
string syair(ifstream &file, ofstream &outFile, string artisttitle) {
  ...
  if (1 < string(str.c_str()).length()) {
    ...
    for (unsigned i = 0; i < str.size() - 2; i++) {
      transform(artisttitle.begin(), artisttitle.end(), artisttitle.begin(),
                ::tolower);
      if (str[i + 8] != '-') {
        if (str[i + 8] == artisttitle[i]) {
        } else if ((str[i + 8] == '/') && (str[i + 9] == artisttitle[i + 3]) &&
                   (str[i + 10] == artisttitle[i + 4])) {
  ...
  while (getline(syairraw, str)) {
    ...
    if (lyricstart == true) {
      ...
      while (getline(iss, str, ' ')) {
        ...
        if (4 < string(str.c_str()).length() &&
            string(str.c_str()).substr(string(str.c_str()).length() - 5) ==
                "<br>\r") {
          ...
          outFile << str << endl;
                 ...
                 string(str.c_str()).substr(string(str.c_str()).length() - 4) ==
                     "<div") {
          ...
          break;
                   ...
                   string(str.c_str())
                           .substr(string(str.c_str()).length() - 4) !=
                       "<br>") {
          outFile << str << " ";
        } else if (3 >= string(str.c_str()).length()) {
          outFile << str << " ";
