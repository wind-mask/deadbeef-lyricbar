...
vector<string> azlyrics_get_songs(string song, string artist) {
  ...
  string lrc = "/lrc/";
  string end_url_search =
      deadbeef->conf_get_str_fast("lyricbar.end_url_search", "");
  string empty_search = "Sorry, your search returned <b>no results</b>";
  struct curl_slist *slist = NULL;
  slist = curl_slist_append(
      slist, "User-Agent: Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 "
  ...
  string url = "https://search.azlyrics.com/search.php?q=" +
               specialforplus(song.c_str()) + "+" +
               specialforplus(artist.c_str()) + end_url_search;
  ...
  if (bulk_results.find(empty_search) == std::string::npos) {
    ...
    for (size_t i = 0; i < results.size(); i++) {
      if (results[i].find(text_left) != std::string::npos) {
        ...
        //				Artist,song,Album (empty), url to
        // lyrics.
...
struct parsed_lyrics azlyrics(string song, string artist) {
  ...
  vector<string> results;
  results = azlyrics_get_songs(specialforplus(song.c_str()),
                               specialforplus(artist.c_str()));
  //	for(int i = 0; i < results.size(); i+=3) {
  //		cout << "artist: " << results[i] << " Song: " << results[i+1] <<
  //" url: " << results[i+2] << "\n";
  //	}
