...
static gboolean _pop(GtkTextView *text_view, GtkWidget *popup,
                     gpointer user_data) {
  ...
  g_signal_connect_after(popup_config, "activate", G_CALLBACK(on_button_config),
                         user_data);
  g_signal_connect_after(popup_search, "activate", G_CALLBACK(on_button_search),
                         user_data);
  g_signal_connect_after(popup_edit, "activate", G_CALLBACK(on_button_edit),
                         user_data);
  return TRUE;
}
...
DB_plugin_action_t remove_action = {.name = "remove_lyrics",
...
static DB_plugin_action_t *lyricbar_get_actions() {
  ...
  while (current) {
    ...
    if (deadbeef->pl_is_selected(current) &&
        is_cached(deadbeef->pl_find_meta(current, "artist"),
...
static ddb_gtkui_widget_t *w_lyricbar_create(void) {
  ...
  GList *children =
      gtk_container_get_children(GTK_CONTAINER(widget->base.widget));
  gtkui_plugin->w_override_signals(widget->base.widget, widget);
  g_signal_connect(children->data, "populate_popup", G_CALLBACK(_pop),
                   children->data);
...
static int lyricbar_connect() {
  ...
  gtkui_plugin->w_reg_widget("Lyricbar", 0, w_lyricbar_create, "lyricbar",
                             NULL);
  return 0;
}
...
DDB_LYRICS_EXPORT DB_plugin_t *
ddb_lyricbar_gtk2_load(DB_functions_t *ddb) {
#else
DDB_LYRICS_EXPORT DB_plugin_t *
ddb_lyricbar_gtk3_load(DB_functions_t *ddb) {
#endif
  deadbeef = ddb;
...
static DB_misc_t plugin = {
    ...
    .plugin.descr = "Lyricbar plugin for DeadBeeF audio player.\nPlugin for "
                    "DeaDBeeF audio player that fetches and shows the song’s "
                    "with scroll on sync lyrics. \n",
    ...
    .plugin.configdialog =
        ...
        "property \"End url search (AZlyrics) (&x=...): \" entry "
        "lyricbar.end_url_search \"\"; \n"
