...
void on_Apply_clicked(GtkButton *b) {
  ...
  deadbeef->conf_set_int(
      "lyricbar.vpostion",
      gtk_spin_button_get_value(GTK_SPIN_BUTTON(Highlight_line)));
  deadbeef->conf_set_int("lyricbar.border",
                         gtk_spin_button_get_value(GTK_SPIN_BUTTON(Edge)));
  printf("%s \n", "Apply clicked");
  get_tags();
...
void set_chooser_color(GtkColorButton *chooser, const char *hexColor) {
  ...
  const GdkRGBA *color =
      new GdkRGBA{Double_red, Double_green, Double_blue, Alpha};
  gtk_color_chooser_set_rgba(GTK_COLOR_CHOOSER(chooser), color);
}

extern "C" int on_button_config(GtkMenuItem *menuitem, gpointer user_data) {
  ...
  //	Widgets creation:
  Background_color =
      GTK_COLOR_BUTTON(gtk_builder_get_object(builder, "Background_color"));
  Highlight_color =
      GTK_COLOR_BUTTON(gtk_builder_get_object(builder, "Highlight_color"));
  Text_color = GTK_COLOR_BUTTON(gtk_builder_get_object(builder, "Text_color"));
  Highlight_line =
      GTK_SPIN_BUTTON(gtk_builder_get_object(builder, "Highlight_line"));
  ...
  Align_center =
      GTK_RADIO_BUTTON(gtk_builder_get_object(builder, "Align_center"));
  Align_right =
      GTK_RADIO_BUTTON(gtk_builder_get_object(builder, "Align_right"));
  ...
  Background_color_label =
      GTK_LABEL(gtk_builder_get_object(builder, "Background_color_label"));
  Text_color_label =
      GTK_LABEL(gtk_builder_get_object(builder, "Text_color_label"));
  Highlight_color_label =
      GTK_LABEL(gtk_builder_get_object(builder, "Highlight_color_label"));
  ...
  Highlight_line_label =
      GTK_LABEL(gtk_builder_get_object(builder, "Highlight_line_label"));
  ...
  g_signal_connect(Background_color, "color-set",
                   G_CALLBACK(on_Background_color_color_set), NULL);
  g_signal_connect(Highlight_color, "color-set",
                   G_CALLBACK(on_Highlight_color_color_set), NULL);
  g_signal_connect(Text_color, "color-set", G_CALLBACK(on_Text_color_color_set),
                   NULL);
  ...
  g_signal_connect(Check_bold, "toggled", G_CALLBACK(on_check_bold_toggled),
                   NULL);
  g_signal_connect(OK, "clicked", G_CALLBACK(on_OK_clicked), ConfigWindow);
  g_signal_connect(Cancel, "clicked", G_CALLBACK(on_Cancel_clicked),
                   ConfigWindow);
  ...
  set_chooser_color(Highlight_color, deadbeef->conf_get_str_fast(
                                         "lyricbar.highlightcolor", "#571c1c"));
  set_chooser_color(Text_color, deadbeef->conf_get_str_fast(
                                    "lyricbar.regularcolor", "#000000"));

  // Saved value to spin buttons:
  gtk_spin_button_set_value(Highlight_line,
                            deadbeef->conf_get_int("lyricbar.vpostion", 50));
  gtk_spin_button_set_value(Edge,
                            deadbeef->conf_get_int("lyricbar.border", 22));
...
string specialtoplus(const char *text) {
  ...
  // {
  //     if( (counter[i] < 'A' ||  counter[i] > 'Z') &&  (counter[i] < 'a' ||
  //     counter[i] > 'z') &&  (counter[i] < '0' ||  counter[i] > '9')) {
  //         counter[i] = '+';
  // 	}
  // }
  return counter;
...
extern "C" int on_button_search(GtkMenuItem *menuitem, gpointer user_data) {
  ...
  Column_artist =
      GTK_TREE_VIEW_COLUMN(gtk_builder_get_object(builder, "Column_artist"));
  Column_song =
      GTK_TREE_VIEW_COLUMN(gtk_builder_get_object(builder, "Column_song"));
  Column_album =
      GTK_TREE_VIEW_COLUMN(gtk_builder_get_object(builder, "Column_album"));
  Column_source =
      GTK_TREE_VIEW_COLUMN(gtk_builder_get_object(builder, "Column_source"));
  Column_searchid =
      GTK_TREE_VIEW_COLUMN(gtk_builder_get_object(builder, "Column_searchid"));
  ...
  if (strcmp(locale_lang, "zh_CN.UTF-8") == 0) {
    ...
    gtk_label_set_text(
        PreViewLyrics,
        (const gchar *)"Ventana de previsualización de las letras");
  }
  ...
  gtk_builder_connect_signals(builder, NULL);
  g_signal_connect(SongsTree, "row-activated",
                   G_CALLBACK(on_row_double_clicked), selection);
...
string apply_delay(string lyrics, int delay) {
  ...
  while (end != -1) {
    ...
    if (line.length() > 9 && (line.at(0) == '[') && (line.at(3) == ':') &&
        (line.at(6) == '.')) {
      int seconds;
      int minutes;
      minutes = (line.at(1) - 48) * 10 + (line.at(2) - 48);
      seconds = (line.at(4) - 48) * 1000 + (line.at(5) - 48) * 100 +
                (line.at(7) - 48) * 10 + (line.at(8) - 48) + delay;
...
void update_text_textview(string text) {
  ...
  gtk_text_buffer_insert(GTK_TEXT_BUFFER(refBuffer), &Start_iter, text.c_str(),
                         -1);
}
...
string get_text_textview() {
  ...
  gtk_text_buffer_get_start_iter(GTK_TEXT_BUFFER(refBuffer), &Start_iter);
  gtk_text_buffer_get_end_iter(GTK_TEXT_BUFFER(refBuffer), &Finish_iter);
  string text = gtk_text_buffer_get_text(GTK_TEXT_BUFFER(refBuffer),
                                         &Start_iter, &Finish_iter, false);
  return text;
}
...
void on_Insert_timestamp_clicked(GtkButton *b) {
  ...
  time = "\n[" + time + "]";
  gtk_text_buffer_get_iter_at_mark(refBuffer, &iter,
                                   gtk_text_buffer_get_insert(refBuffer));
...
extern "C" int on_button_edit(GtkMenuItem *menuitem, gpointer user_data) {
  ...
  //	Widgets creation:
  Text_view_lyrics_editor =
      GTK_TEXT_VIEW(gtk_builder_get_object(builder, "Text_view_lyrics_editor"));
  ...
  Insert_timestamp =
      GTK_BUTTON(gtk_builder_get_object(builder, "Insert_timestamp"));
  ...
  Sync_or_not_label =
      GTK_LABEL(gtk_builder_get_object(builder, "Sync_or_not_label"));
  ...
  g_signal_connect(Pause_play, "clicked", G_CALLBACK(on_Pause_play_clicked),
                   NULL);
  ...
  g_signal_connect(Plus_time, "clicked", G_CALLBACK(on_Plus_time_clicked),
                   NULL);
  g_signal_connect(Minus_time, "clicked", G_CALLBACK(on_Minus_time_clicked),
                   NULL);
  g_signal_connect(Apply_offset, "clicked", G_CALLBACK(on_Apply_offset_clicked),
                   NULL);
  g_signal_connect(Insert_timestamp, "clicked",
                   G_CALLBACK(on_Insert_timestamp_clicked), NULL);
  g_signal_connect(Edit_apply, "clicked", G_CALLBACK(on_Edit_apply_clicked),
                   NULL);
  g_signal_connect(Edit_cancel, "clicked", G_CALLBACK(on_Edit_cancel_clicked),
                   EditWindow);
  g_signal_connect(Edit_OK, "clicked", G_CALLBACK(on_Edit_OK_clicked),
                   EditWindow);
