...
// static const string lyrics_dir = (home_cache ? string(home_cache) :
// string(getenv("HOME")) + "/.cache") + "/deadbeef/lyrics/";
...
char *locale_lang1 = setlocale(LC_CTYPE, NULL);
// For debug:
...
void write_synced(DB_playItem_t *it) {
  ...
  if (lrc.position.size() > 2) {
    ...
    if (!linessizes.empty()) {
      for (int j = 0;
           j < (int)((lrc.position[linessizes[1] + 1] - pos) /
                     (lrc.position[linessizes[1] + 1]) * linessizes[0]);
           j++) {
        padding.append("\n");
      }
    }
    ...
    if (((!linessizes.empty()) && (presentpos - linessizes[1]) > 0)) {
      ...
      for (int j = 0;
           j <
           (int)(((lrc.position[presentpos + 1] - pos) /
                  (lrc.position[presentpos + 1] - lrc.position[presentpos])) *
                 (linessizes[presentpos - linessizes[1] + 5] - 1));
           j++) {
        padding.append("\n");
      }
    }
    ...
    for (unsigned i = minimuntopad;
         lrc.position[i + 1] < pos && i < lrc.position.size() - 2; i++) {
...
void save_meta_data(DB_playItem_t *playing_song, struct parsed_lyrics lyrics) {
  ...
  const char *UNSYNCEDLYRICS =
      deadbeef->pl_find_meta(playing_song, "UNSYNCEDLYRICS");
  deadbeef->pl_unlock();
  if (lyrics.sync == true) {
    if (SYLT) {
    ...
    deadbeef->pl_add_meta(playing_song, "UNSYNCEDLYRICS",
                          lyrics.lyrics.c_str());
  }
  deadbeef->pl_lock();
...
std::string replace_string(string subject, const string &search,
...
inline string cached_filename(string artist, string title) { return 0; }

extern "C" bool is_cached(const char *artist, const char *title) {
  return false;
}

extern "C" void ensure_lyrics_path_exists() {}
...
void update_lyrics(void *tr) {
  ...
  if (strcmp(locale_lang1, "zh_CN.UTF-8") == 0) {
    set_lyrics(track, "", "", _("加载中..."), "");
  } else {
    set_lyrics(track, "", "", _("Loading..."), "");
  }
  //   ebug_out << "Loading..." << std::endl;
  ...
  // // debug_out << "Loaded kugou_lyrics.lyrics" << kugou_lyrics.lyrics <<
  // std::endl; if (kugou_lyrics.lyrics != "") {
  ...
  //     return;
  // }
  struct parsed_lyrics music163_lyrics = {"", false};

  music163_lyrics = music163(title, artist);
  if (music163_lyrics.lyrics != "") {
    // save_next_to_file(music163_lyrics, track);
    if (music163_lyrics.sync) {
      chopset_lyrics(track, music163_lyrics.lyrics);
    } else {
      set_lyrics(track, "", "", music163_lyrics.lyrics, "");
    }
    sync_or_unsync(syncedlyrics);
    // save_meta_data(track, music163_lyrics);
    return;
  }
