...
vector<string> spotify_get_songs(string song, string artist) {
  ...
  url = "https://api.spotify.com/v1/search?q=track%3A" + song + "+artist%3A" +
        artist + "&type=track";
  _songs = split(text_downloader(slist, url), ":");
  string album;

  //	Search for Artist, Album, Track and Spotify ID.
  for (size_t i = 0; i < _songs.size(); i++) {
    vector<string> songs_split;
    songs_split = split(_songs[i], "\n");
    string temp;
    for (size_t j = 0; j < songs_split.size(); j++) {
      //			Artist (we only want one, so avoid to add more
      // with (songs_clear.size() % 4 == 0)):
      if ((songs_split[j] == "          \"type\" ") &&
          (songs_clear.size() % 4 == 0)) {
